...
export const QueuePage: React.FC = () => {
  ...
  const [queue] = useState(
    new Queue<{ value: string; state: ElementStates }>(7)
  );
  const [queueArr, setQueueArr] = useState<
    { value: string; state: ElementStates }[]
  >(queue.getData());
    ...
    <SolutionLayout title="Очередь">
      <div className={style.wrapper}>
        ...
        <Button
          ...
          disabled={isLoader.deleteButton || !inputValue || queue.isFullQueue()}
          extraClass={`mr-6 ${style.addButton}`}
        />
        <Button
          text="Удалить"
          onClick={onDeleteButtonClick}
          isLoader={isLoader.deleteButton}
          disabled={isLoader.addButton || queue.isEmpty()}
          extraClass={`mr-40 ${style.deleteButton}`}
        />
        <Button
          onClick={onCleanButtonClick}
          text="Очистить"
          disabled={
            isLoader.addButton || isLoader.deleteButton || queue.isEmpty()
          }
          extraClass={style.cleanButton}
        />
      </div>
      <div className={style.letters}>
        {queueArr ? (
          queueArr.map(
            (item: { value: string; state: ElementStates }, index: number) => (
              <Circle
                key={index}
                index={index}
                letter={item.value}
