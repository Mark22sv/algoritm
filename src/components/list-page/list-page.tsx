...
import { IElementList, INode } from "../../types/linkedList";
...
export const ListPage: React.FC = () => {
  ...
  const onDeleteTailButtonClick = async () => {
    ...
    setNode({
      ...node,
      delete: true,
      value: linkedList.findByIndex(linkedList.getLength() - 1).value,
    });
    setElementIndex(1);
  ...
  const onDeleteByIndexButtonClick = async () => {
    ...
    setNode({
      ...node,
      delete: true,
      value: linkedList.findByIndex(+inputValue.index).value,
    });
  ...
  return (
    <SolutionLayout title="Связный список">
      <div className={styles.wrapper}>
        ...
        />
        <Button
          onClick={onAddHeadButtonClick}
          isLoader={isLoader.loader && isLoader.button === "addHead"}
          text="Добавить в head"
          disabled={!inputValue.value || isLoader.loader}
          extraClass={styles.button}
        />
        <Button
          onClick={onAddTailButtonClick}
          isLoader={isLoader.loader && isLoader.button === "addTail"}
          text="Добавить в tail"
          disabled={!inputValue.value || isLoader.loader}
          extraClass={styles.button}
        />
        <Button
          onClick={onDeleteHeadButtonClick}
          isLoader={isLoader.loader && isLoader.button === "deleteHead"}
          text="Удалить из head"
          disabled={linkedList.isEmpty() || isLoader.loader}
          extraClass={styles.button}
        />
        <Button
          onClick={onDeleteTailButtonClick}
          isLoader={isLoader.loader && isLoader.button === "deleteTail"}
      ...
      <div className={styles.wrapperIndex}>
        ...
        />
        <Button
          onClick={onAddByIndexButtonClick}
          isLoader={isLoader.loader && isLoader.button === "addByIndex"}
          text="Добавить по индексу"
          disabled={
            !inputValue.index || +inputValue.index > linkedList.getLength() - 1
          }
          extraClass={styles.buttonIndex}
        />
        <Button
          onClick={onDeleteByIndexButtonClick}
          isLoader={isLoader.loader && isLoader.button === "deleteByIndex"}
          ...
          listArr.map((item: any, index: number) => (
            <li className={styles.list} key={index}>
              ...
              <Circle
                ...
                tail={!item.next && !node.delete ? "tail" : ""}
                head={index === 0 && !node.addNode ? "head" : ""}
              />
              {node.delete &&
                linkedList.getLength() - elementIndex === index && (
                  <Circle
